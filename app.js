var Calculadora = {
	/* Variables*/
	mostrar: document.getElementById("display").innerHTML,
	decimal: 0,
	signo: 0,
	controlen: 8,
	stop: 0,
	...
	igual: function(){
		...
		switch(this.opcion){
			case 1:
					if(this.auxestado == 0){
						this.auxnum = Number(this.mostrar),
						this.mostrar = this.num1 + Number(this.mostrar),
						this.auxestado = 1,
						this.num1 = 0;
					}else{
						this.mostrar = Number(this.mostrar)+this.auxnum;
					}
				break;
			case 2:
					if(this.auxestado == 0){
	...
	viewnum: function(valor){
		...
		if(this.mostrar.length < this.controlen){
			if(this.mostrar != "0"){
				this.mostrar += valor;
			}else if(valor != 0){
				this.mostrar = "",
	...
	sign: function(){
		...
		if(this.mostrar != 0){
			if(this.signo == 0){
				...
				this.mostrar = this.mostrar.slice(1);
				this.signo = 0;
			}
		}
		this.viewdisplay();
	...
	viewdisplay: function(){
		if(this.mostrar.toString().length > this.controlen){
			this.mostrar = this.mostrar.toString().substring(0,8);
		}
		document.getElementById("display").innerHTML = this.mostrar;
	}		
}
Calculadora.inicio();
